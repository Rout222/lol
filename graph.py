...
limit = 100000
offset_inicial = 0
ultima_parada_offset = -1 # caso seja necessário parar , utilize o offset do log aqui.
c.execute("SELECT count(*) FROM `players`")
count = (c.fetchall()[0][0]) - offset_inicial
offrange = range(ultima_parada_offset+1, math.ceil(count/limit))
with click.progressbar(offrange, label='Partidas Processadas') as offbar:
	for off in offbar:
				...
				for _, y in x['list'].items():
					if(y['count'] > 0 or y['jogoucontra'] > 0):
						...
						if(len(c.fetchall()) == 0):
							...
							c.execute("UPDATE `arcs` SET `value`=`value`+\"{}\",`jogoucontra`=`jogoucontra`+\"{}\",`ganhoucontra`=`ganhoucontra`+\"{}\",`ganhoujunto`=`ganhoujunto`+\"{}\" where `source_id` =\"{}\" AND `target_id` =\"{}\" ".format(y['count'],y['jogoucontra'],y['ganhoucontra'],y['win'],x['id'],y['id']))
